...
export default function AuthPortal({ isOpen, onClose, onAuthSuccess }: AuthPortalProps) {
  ...
  const handleGoogleSso = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setSuccessMsg('');
      try {
        const user = await auth.googleLogin(tokenResponse.credential || tokenResponse.access_token);
                  ...
                  <form onSubmit={handleEmailLogin} className="space-y-4 text-left">
                    ...
                    <div className="pt-2">
                      <button
                        ...
                        disabled={loading}
                        className="w-full bg-zinc-100 hover:bg-white text-zinc-900 font-bold text-xs py-3 px-4 rounded-xl transition flex items-center justify-center gap-2 cursor-pointer shadow-[0_2px_10px_rgba(0,0,0,0.2)]"
                      >
